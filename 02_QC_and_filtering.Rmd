...
We now perform UMAP and plot them using the different resolutions. Due to working with brain samples, where the cell population is quite diverse (microglia and neuron subtypes) and that we want to perform analyses on specific cell types, we will probably need a high resolution.
...
**Saving the final Seurat object**

Now all the reductions are included in the Seurat object, as well as the graphs used for clustering.

We are using Seurat v5 where the RNA assay is an Assay5 object, so we need @layers$counts instead of just @counts to access the different data.

```{r}
# ms_data.filtered@assays$RNA@layers$counts
# ms_data.filtered@assays$RNA@layers$data
# ms_data.filtered@assays$RNA@layers$scale.data
```

```{r}
ms_data.filtered@reductions$umap
```

Save the object in the common folder:

```{r}
saveRDS(ms_data.filtered, file = "/home/projects/exam_2026_22102/group4/data/ms_filtered_full.rds", compress = "xz")
```
...
# --- (Optional) basic QC filter first (use paper thresholds) ---
ms <- subset(
  ms_data,
  subset =
    nFeature_RNA >= 500 &
...
counts <- ms[["RNA"]]@layers$counts
keep_genes <- Matrix::rowSums(counts > 0) >= 3
ms <- subset(ms, features = names(keep_genes)[keep_genes])

mt_genes <- grep("^MT-", rownames(ms), value = TRUE)
ms <- subset(ms, features = setdiff(rownames(ms), mt_genes))
...
```{r}
DimPlot(ms, reduction = "tsne", label = TRUE)
...
```{r}
ms_data.new <- ms
saveRDS(ms_data.new, file = "/home/projects/exam_2026_22102/group4/data/ms_filtered_new.rds", compress = "xz")
```
